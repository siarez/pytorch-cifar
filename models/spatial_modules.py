...
class SpatialConv2d(torch.nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1, bias=False, shape_passthrough=False, concat_distance=False):
        ...
        self.bn_distance = BatchNorm2d(out_channels, affine=True)  # experiment
        ...
        self.register_buffer('input_shapes', None)
        # utils.clip_grad_value_(self.parameters(), 0.1)
        
    def forward(self, x):
        # Todo: Variance less than this doesn't really makes sense. I'm not sure if clamping is the right approach to fix it though. I should
        #  see what is driving the variance towards that negative numbers. It doesn't make sense, because even if it is just fitting noise, the
        #  mean of the noise can't be less than zero.
        self.shapes_kernel[:, :, 2:4, :, :].data.clamp_(0.1)  # experiment
        if not self.shape_passthrough:
            if not self.concat_distance:
                ...
                # self.input_shapes = unfold(x_padded[:, -5:, ...], self.k).view(-1, 5, self.k * self.k, num_of_win).clone()
                shape_windows = unfold(x_padded[:, -5:, ...], self.k).view(-1, 5, self.k * self.k, num_of_win)
                shape_windows[:, -5:-3, :, :] = shape_windows[:, -5:-3, :, :] - shape_windows[:, -5:-3, self.win_center_idx:self.win_center_idx + 1, :]
                self.input_shapes = shape_windows.clone().detach()
                ...
                shape_difference = torch.norm(shapes_delta, p=2, dim=2) # / self.query_scale_factor  # experiment: try using different norms
                ...
                # Need to `clone()` because more than one element of the unfolded tensor may refer to a single memory location.
                # This would make subtraction of the local window centers to work properly when writing back to `shape_windows`
                shape_windows = unfold(x_padded[:, -5:, ...], self.k).view(-1, 5, self.k * self.k, num_of_win).clone()  # .detach() run 2020-01-06-17-16-40 had detach. variance still goes towards negative.
                shape_windows[:, -5:-3, :, :] = shape_windows[:, -5:-3, :, :] - shape_windows[:, -5:-3, self.win_center_idx:self.win_center_idx + 1, :]
                self.input_shapes = shape_windows.clone().detach()
                shapes_delta = (shape_windows.unsqueeze(1) - self.shapes_kernel).view(-1, self.out_channels, 5 * (self.k ** 2), num_of_win)
                # shapes_delta = (torch.exp((shape_windows.unsqueeze(1) - self.shapes_kernel)**2)).view(-1, self.out_channels, 5*self.k**2, num_of_win)
                # shape_difference = shapes_delta.sum(dim=2)
                shape_difference = torch.norm(shapes_delta, p=2, dim=2)  # / self.query_scale_factor  # experiment: l1 norm, could be other norms
                # shape_difference_soft = torch.softmax(-shape_difference)  # bigger difference results in smaller multiplier
                # shape_difference_soft = shape_difference_soft*self.shapes_channel_weight # learnable scale factor
                self.shape_distance_weighted = fold(shape_difference, (x_padded.shape[2] - self.k + 1, x_padded.shape[3] - self.k + 1), (1, 1))
                # shape_distance_weighted = self.sm2d(-shape_distance_weighted * self.concordance_coef2)
                ...
                shape_attended_features = self.bn2d(conv_out)
                output = torch.cat([shape_attended_features, x[:, -5:, ...]], dim=1)
        else:
            conv_out = self.bn2d(self.conv(self.pad2d(x[:, :-5, ...])))
...
class SpatialMaxpool2d_2(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        ellipse_centers = unfold(x_shape[:, 0:2, ...], (self.k, self.k), stride=shape_stride)
        window_means = torch.sum(ellipse_centers.view(-1, 2, self.k*self.k, num_of_win2) * window_shape_query, dim=2)
        # Part_1 is contribution of variances of ellipses. Part_2 is the contribution of how far the mean of each ellipse is
        # from the mean of the window.
        window_var_part_1 = torch.sum(
            unfold(x_shape[:, 2:4, ...], (self.k, self.k), stride=shape_stride).view(-1, 2, self.k*self.k, num_of_win2) * window_shape_query, dim=2)
        window_var_part_2 = (((unfold(x_shape[:, 0:2, ...], (self.k, self.k), stride=shape_stride).view(-1, 2, self.k*self.k, num_of_win2) - window_means.
                               unsqueeze(2)) ** 2) * window_shape_query).sum(dim=2)
        window_var = window_var_part_1 + window_var_part_2
        # Part_1 is contribution of covariances of ellipses. Part_2 is the contribution of how far the mean of each ellipse is
        # from the mean of the window.
        window_covar_part_1 = torch.sum(
            unfold(x_shape[:, 4:, ...], (self.k, self.k), stride=shape_stride).view(-1, 1, self.k * self.k, num_of_win2) * window_shape_query, dim=2)
        window_covar_part_2 = (((unfold(x_shape[:, 0:1, ...], (self.k, self.k), stride=shape_stride).view(-1, 1, self.k * self.k, num_of_win2) - window_means[:,0:1,:].unsqueeze(2)) *
                                (unfold(x_shape[:, 1:2, ...], (self.k, self.k), stride=shape_stride).view(-1, 1, self.k * self.k, num_of_win2) - window_means[:,1:2,:].unsqueeze(2))) *
...
class DummyLayer(torch.nn.Module):
    ...
    def extra_repr(self):
        return 'Dummy layer. Doesn\'t to anything'
...
class SpatialBatchNorm2d(torch.nn.Module):
    ...
    def __init__(self, channels, affine=True):
        ...
        self.bn2d = BatchNorm2d(channels, affine=affine)
    ...
    def extra_repr(self):
        return self.bn2d.extra_repr()


class SpatialReLU(torch.nn.Module):
    """Does ReLu on feature channels but doesn't touch shape channels"""
    def __init__(self, inplace=False):
        super(SpatialReLU, self).__init__()
        self.relu = torch.nn.ReLU(inplace=inplace)

    def forward(self, x):
        x_relu = self.relu(x[:, :-5, ...])
        return torch.cat([x_relu, x[:, -5:, ...]], dim=1)
