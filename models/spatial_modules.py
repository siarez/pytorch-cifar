...
class SpatialConv2d(torch.nn.Module):
    ...
    """
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1, bias=False, shape_passthrough=True):
        ...
        self.bn2d = BatchNorm2d(out_channels)
...
class SpatialMaxpool2d(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        # be experimented with. I think it has to do with random walks.
        shape_weights = torch.norm(max_unpooled, dim=1, keepdim=True) # / torch.tensor(in_channels - 5, requires_grad=False).type(torch.FloatTensor).sqrt()
